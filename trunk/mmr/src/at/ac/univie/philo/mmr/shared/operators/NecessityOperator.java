...
public class NecessityOperator implements IModalOperator, IsSerializable {
	...
	public EvaluationResult evaluate(Expression scope, Universe universe,
		...
		Comment comment = new Comment("Box: We have to check for all Worlds which are accessible from World "+ initialWorld.getName()+ " that the Expression holds: "+scope);
		try {
			for (World w : universe.getAccessibleWorlds(initialWorld)) {
				...
				if(scopeResult.isSentence()) {
					if (scopeResult.getValue().getValue().equals(TruthValue.FALSE)) {
						comment.addLine("Box: In World "+w.getName()+" the Scope "+scope+" does not hold. That means we found a counter-example and the Box-Expression evaluates to false.");
						return new EvaluationResult(new TruthExpression(false), comment);
					} else {
						comment.addLine("Box: In World "+w.getName()+" the Scope "+scope+" hold. Lets look in the other worlds.");
					}
				} else {
					hasFreeVars = true;
		...
		comment.addLine("Box: We checked for all Worlds that the Scope "+scope+" hold. The Box-Expression therefore evalutes to true.");
